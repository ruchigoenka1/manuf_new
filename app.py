...
with tab8:
    ...
    st.info("💡 **Tip:** Use the 'Node ID' column to link processes together. To connect multiple suppliers to a single assembly point, enter multiple IDs separated by commas in the 'Preceding Node(s)' column (e.g., 'N1, N2'). The row order does not matter!")
    ...
    if not edited_df.empty:
        ...
        # Use a tight margin so the shapes wrap closely around the text. 
        # This prevents the whole image from becoming too wide and scaling down.
        dot_code += '  node [fontname="Helvetica", fontsize=14, margin="0.1,0.1"];\n'
        dot_code += '  edge [penwidth=2.0];\n'
        ...
        for index, row in edited_df.iterrows():
            ...
            # Filter out empty rows generated by the data editor
        ...
        # Wrapped in a collapsible expander and set to full width
        with st.expander("🗺️ 2. Visual Network Map & Cash Cycle", expanded=True):
...
with tab9:
   ...
   # --- SIMULATION FUNCTION HELPER ---
    ...
    # Calculate Transit Inventory (Ownership transfers at dispatch)
    avg_transit_inv = avg_demand_t9 * lead_time_t9
    ...
    dso = credit_given_t9
    dpo = credit_rx_t9 # Full credit period recognized
    ccc = dio + dso - dpo
    ...
    ccc_c1.metric("Transit Inventory Days", f"{lead_time_t9:.1f} Days", f"Value: ${avg_transit_val:,.0f}", delta_color="off")
    ...
    if st.button("🚀 Run Multi-Scenario Analysis", type="primary", use_container_width=True, key="t9_run_scen_btn"):
        ...
        for sc in scenario_inputs:
            ...
            # Ownership transfers at dispatch, so transit days = lead time
            avg_transit_inv = avg_demand_t9 * sc["Lt"]
            total_owned_inv = res['avg_inventory'] + avg_transit_inv
            
            dio = total_owned_inv / avg_demand_t9 if avg_demand_t9 > 0 else 0
            dso = credit_given_t9
            dpo = sc["Crx"] # Full credit period recognized
            ccc = dio + dso - dpo
